<div id="version">
    <ul>
        ...
        <li>
            <h5><?=substr($version['video']['file_name'],0,-4)?></h5>
        </li>
    </ul>         	
        <?php /*$still = substr(str_replace(' ','_',$file),0,-3)."png";
        $still = "uploads/".$title.'/thumbnails/'.$still;*/ ?>
        <?php if ( ! strpos($_SERVER["HTTP_USER_AGENT"],'iPhone')): ?>
                <p id="loadStatus" style="text-align:left;color:#c6d73d;">Cargando por favor espere...</p></br>
        <?php endif; ?>            
    <table class="vercom">
        <tr>
            <td rowspan="4">&nbsp;</td>
        ...
        <?php  if (substr($version['video']['file_type'],6,3) === 'mp4' || stripos($_SERVER["HTTP_USER_AGENT"],'Chrome') && ! (stripos($_SERVER["HTTP_USER_AGENT"],'iPhone') || stripos($_SERVER["HTTP_USER_AGENT"],'iPad'))): ?> 
            		
                <?php echo "<video id='videomp4' controls ";
                /*if (base_url($still))
                {
        ...
        <tr>
            <td rowspan="1">
                <textarea rows="17" cols="34" name="commentsv" id="commentsv" onClick="delnote()">
            <?=set_value('commentsv','Por favor utilice este campo para enviarnos sus sugerencias o comentarios.')?>
                </textarea>
            </td>
        </tr>
        <tr>
            <th colspan="1" onClick="goPlay('<?=$version['video']['id']?>')">
                <a href="javascript:void(0)">Play HD</a>
            </th>
            <th colspan="1" onClick="goDown('<?=$version['video']['id']?>')">
                <a href="javascript:void(0)">Download</a>
            </th>
            <td colspan="1"><input type="button" value="Enviar" id="sendmail" name="Send" 
            onclick="savecomments('<?=$version['video']['id']?>','<?=substr($video,8)?>')"/>
            </td>
        </tr>		
    </table>
</div>
