<div id="list"><h1><?php echo ucfirst(strtolower($title)) ?></h1> 
    ...
    <div class="order_button">
        <a href="javascript:void(0)" data-toggle="tooltip" title="Inicialmente ordenados por fecha, a partir de la mas reciente." style="color:#ffffff">( ? ) Ordenar por:</a>
        ...
        <?php if ($type === 'admin') echo set_value('commentsv' . $j, $comments) ?>
            ...
            <?php endif; ?>
                ...
                <td class="comments" colspan="1">
                    <input type="button" value="Enviar" name="Send<?=$j?>" onclick="savecomments('<?=$versions["videos"][$j]["id"]?>','<?=$j?>')" />
                </td>
            </tr>
        </table>
    <?php endfor; ?>
