...
class Clientes_model extends CI_Model {
	...
	public function get_clips($clipstype)
	{
            $this->db->select('fname, img_name, list_order');
            $query = $this->db->get_where('byg_clips', array('type' => $clipstype));
	...
	public function upload_clip()
	{
            if ($_FILES['file']['size'] !== 0)
            {
                ...
                $listorder = $query->row_array();
                $newlistorder = $listorder['list_order']+1;
                $file_type = substr($_FILES['file']['type'],0,5);
                if ($file_type == 'image')
                {
                ...
                else
                {
                        ...
                        $this->db->like('type',$tipo);
                        $this->db->like('fname',$fname);
                        $this->db->from('byg_clips');
                        $eclip = $this->db->count_all_results();
                }	
                $data = array(
                        'type'  => $tipo, 
                        'fname' => $fname,
                ...
                $config['upload_path'] = realpath(FCPATH.'../video/'.$tipo); 
